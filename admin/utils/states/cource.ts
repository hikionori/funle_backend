...
export const useCourseStore = create((set, get: any) => ({
    ...
    editNode: (levelIndex: number, nodeIndex: number, ids: string[], title: string, mini_image: string, type_: string, n_of_tests: number | null) => {
        const levels = get().levels;
        const level = levels[levelIndex];
    ...
    deleteNode: (levelIndex: number, nodeIndex: number) => {
        const levels = get().levels;
        const level = levels[levelIndex];
        const filteredLevel = level[1].filter((node: any) => node.index !== nodeIndex);
